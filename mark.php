<?php
	...
	if(isset($_GET["rno"]))
	{
		$sql="select * from student where adm_no='{$_GET["rno"]}'";
		$res=$db->query($sql);
		if($res->num_rows<=0)
		{
			...
			<div id="section">
				...
				<div class="content">
					...
					<?php
						...
						{
							$sq="insert into mark (adm_no,SUB,MARK,TERM,form) values ('{$_POST["regno"]}','{$_POST["sub"]}','{$_POST["mark"]}','{$_POST["etype"]}','{$_POST["class"]}')";
							if($db->query($sq))
							{
